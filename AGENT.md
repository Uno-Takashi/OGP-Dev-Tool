...
OGP Dev Tool is a Chrome extension providing live preview of how a web page's Open Graph Protocol (OGP) metadata renders on social networks and UI component libraries (Twitter/X, Facebook, shadcn/ui, Ant Design, Material UI).
...
│   └── locales/en|ja|zh/ # translation.json per language
...
└── shared/               # Framework-agnostic utilities
    ...
    └── utils/            # (kept for non-hook utilities)
```
...
- **Options Page** — Language + theme prefs saved to `chrome.storage.sync`
...
```typescript
{
  ogp: { tags: OGPTag[]; isLoading: boolean; error: string | null };
  ui:  { isDarkMode: boolean; language: 'en' | 'ja' | 'zh' };
}
```

## Internationalization

Languages: **English** (`en`), **Japanese** (`ja`), **Chinese Simplified** (`zh`)
Files: `src/i18n/locales/{lang}/translation.json`

When adding UI strings, add the key to **all 3** locale files.
```typescript
const { t } = useTranslation();
t('panel.reload') // → "Reload OGP Contents"
```
...
4. Add `preview.your.title` to all 3 translation files
...
| `publish.yml` | GitHub Release published | Auto-publish to Chrome Web Store |
| `release.yml` | Manual dispatch | Create GitHub release + zip artifact |
